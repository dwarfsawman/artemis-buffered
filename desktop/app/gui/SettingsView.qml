            ...
            Column {
                ...
                AutoResizingComboBox {
                    ...
                    model: ListModel {
                        ...
                        ListElement { text: qsTr("Low latency"); val: 0 }
                        ListElement { text: qsTr("40 ms"); val: 40 }
                        ListElement { text: qsTr("60 ms"); val: 60 }
                        ListElement { text: qsTr("80 ms"); val: 80 }
                        ListElement { text: qsTr("120 ms"); val: 120 }
                    }
                    Component.onCompleted: {
                        for (var i = 0; i < audioBufferListModel.count; i++) {
                            if (audioBufferListModel.get(i).val === StreamingPreferences.audioBufferMs) {
                                currentIndex = i
                                break
                            }
                        }
                    }
                    onActivated: StreamingPreferences.audioBufferMs = audioBufferListModel.get(currentIndex).val
                }
