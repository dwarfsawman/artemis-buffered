...
#define SER_AUDIOBUFFERMS "audiobufferms"
...
void StreamingPreferences::reload()
{
    ...
    // Missing keys retain the original low-latency behavior for existing users.
    audioBufferMs = settings.value(SER_AUDIOBUFFERMS, 0).toInt();
    if (audioBufferMs != 0 && audioBufferMs != 40 && audioBufferMs != 60 &&
            audioBufferMs != 80 && audioBufferMs != 120) {
        audioBufferMs = 0;
    }
...
void StreamingPreferences::save()
{
    ...
    settings.setValue(SER_AUDIOBUFFERMS, audioBufferMs);
