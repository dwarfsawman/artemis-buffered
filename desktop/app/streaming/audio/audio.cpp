...
IAudioRenderer* Session::createAudioRenderer(const POPUS_MULTISTREAM_CONFIGURATION opusConfig)
{
    ...
    if (mlAudio == "sdl") {
        ...
        SoundIoAudioRenderer* renderer = new SoundIoAudioRenderer(m_Preferences->audioBufferMs);
...
#if defined(Q_OS_WIN) && defined(HAVE_SOUNDIO)
    if (m_Preferences->audioBufferMs > 0) {
        // Buffered playback requires the existing libsoundio ring and WASAPI path.
        SoundIoAudioRenderer* renderer = new SoundIoAudioRenderer(m_Preferences->audioBufferMs);
        if (renderer->prepareForPlayback(opusConfig))
            return renderer;
        delete renderer;
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "Buffered audio requires a working libsoundio output device");
        return nullptr;
    }
#endif
