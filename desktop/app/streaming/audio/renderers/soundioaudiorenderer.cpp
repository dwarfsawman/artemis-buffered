...
SoundIoAudioRenderer::SoundIoAudioRenderer(int targetBufferMs)
    : m_OpusChannelCount(0),
      ...
      m_Latency(0),
      m_TargetBufferMs(targetBufferMs),
      m_TargetFrames(0),
      m_Primed(targetBufferMs == 0),
      m_UnderrunCount(0),
      m_SilenceFrames(0),
      m_LastBufferLogTime(0),
      m_Errored(false)
{
...
SoundIoAudioRenderer::~SoundIoAudioRenderer()
{
    ...
    SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION,
                 "Audio buffer final: underruns=%llu silence_frames=%llu",
                 static_cast<unsigned long long>(m_UnderrunCount),
...
bool SoundIoAudioRenderer::prepareForPlayback(const OPUS_MULTISTREAM_CONFIGURATION* opusConfig)
{
    ...
    int packetsToBuffer;
    m_TargetFrames = (opusConfig->sampleRate * m_TargetBufferMs) / 1000;
    ...
    if (m_TargetFrames > 0) {
        // Leave room for arriving packets after the initial target is reached.
        packetsToBuffer = qMax(packetsToBuffer,
                               (m_TargetFrames + opusConfig->samplesPerFrame - 1) /
                               opusConfig->samplesPerFrame + 2);
    }
...
void SoundIoAudioRenderer::sioWriteCallback(SoundIoOutStream* stream, int frameCountMin, int frameCountMax)
{
    ...
    const int bufferedFrames = framesLeft;
    if (!me->m_Primed) {
        if (framesLeft >= me->m_TargetFrames) {
            me->m_Primed = true;
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                        "Audio buffer primed: %d frames (%d ms target)",
                        framesLeft, me->m_TargetBufferMs);
        }
        else {
            // Keep the output callback running, but do not consume the ring yet.
            framesLeft = 0;
        }
    }
    int bytesRead = 0;
    ...
    if (SDL_LogGetPriority(SDL_LOG_CATEGORY_APPLICATION) <= SDL_LOG_PRIORITY_DEBUG) {
        ...
        if (now - me->m_LastBufferLogTime >= 5000) {
            SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION,
                         "Audio buffer: fill=%d frames target=%d primed=%d underruns=%llu silence_frames=%llu",
                         bufferedFrames, me->m_TargetFrames, me->m_Primed,
                         static_cast<unsigned long long>(me->m_UnderrunCount),
                         static_cast<unsigned long long>(me->m_SilenceFrames));
            me->m_LastBufferLogTime = now;
        }
    }
    ...
    for (;;) {
        ...
        if (me->m_Primed && frameCount > framesLeft) {
            me->m_UnderrunCount++;
            me->m_SilenceFrames += frameCount - framesLeft;
        }
